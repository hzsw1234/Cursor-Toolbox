...
Chatbox Toolbox is a Chrome/Edge Manifest V3 browser extension that enhances the web.chatboxai.app (Chatbox AI) web chat experience with MCP tool bridging, session management, and layout enhancements. It is a sister project to [MCP-Gateway](https://github.com/hzsw1234/MCP-Gateway) which provides the MCP server infrastructure. There is no backend server or database — all state lives in `chrome.storage.local`.
...
3. The extension appears as "Chatbox Toolbox 1.1.2"
...
- Content scripts only inject on `https://web.chatboxai.app/*`, so full end-to-end MCP testing requires a Chatbox AI session and a running MCP-Gateway server.
- The generated `web_mcp_cli/background.bundle.js` is gitignored — it must be rebuilt after cloning or pulling.
- Chatbox AI uses a hybrid UI framework (Material UI + Mantine UI + Tailwind CSS) with Virtuoso for virtualized lists. DOM selectors in content scripts target these framework-specific class patterns.
- Chat sessions in Chatbox use `/session/<id>` URL routes (vs `/chat/<id>` in the original Cursor version).
